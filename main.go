...
func (w Walking) Calories() float64 {
	menSpeed := w.meanSpeed() * KmHInMsec
	math := math.Pow(menSpeed,2)
	HeigtImMetr := w.Height / CmInM
	return ((CaloriesWeightMultiplier * w.Weight +(math / HeigtImMetr) * CaloriesSpeedHeightMultiplier * w.Weight) * w.Duration.Hours() * MinInHours)
} // ((CaloriesWeightMultiplier * вес_спортсмена_в_кг + (средняя_скорость_в_метрах_в_секунду**2 / рост_в_сантиметрах)
...
func ReadData(training CaloriesCalculator) string {
	...
	calories := training.Calories()          // получите количество затраченных калорий

	info := training.TrainingInfo()    // получите информацию о тренировке

	info.Calories = calories                 // добавьте полученные калории в структуру с информацией о тренировке
